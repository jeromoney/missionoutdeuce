...
def test_list_team_members_returns_404_for_unknown_team(client):
    response = client.get("/teams/missing-team/members")
...
def test_create_team_member_creates_user_and_membership(client, seeded_team, db_session):
...
def test_create_team_member_reuses_existing_user_for_new_team(
    client, seeded_user, seeded_second_team, db_session
):
    response = client.post(
...
def test_create_team_member_returns_404_for_unknown_team(client):
    response = client.post(
...
def test_update_team_member_returns_404_for_unknown_membership(client, seeded_team):
    response = client.patch(
...
def test_update_team_member_rejects_membership_on_other_team(
    client, seeded_team, seeded_second_team, seeded_user, db_session
):
    membership = (
        db_session.query(TeamMembership)
        .filter_by(user_id=seeded_user.id, team_id=seeded_team.id)
...
def test_delete_team_member_returns_404_for_unknown_membership(client, seeded_team):
    response = client.delete(
...
def test_list_team_devices_empty(client, seeded_team):
