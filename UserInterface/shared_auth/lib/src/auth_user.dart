...
/// A MissionOut user profile returned by [/users/me].
class AuthUser {
  const AuthUser({
  ...
  /// Parses an [AuthUserRead] JSON object from [/users/me].
  ...
  Map<String, dynamic> toJson() => {
        ...
        'team_memberships':
            teamMemberships.map((m) => m.toJson()).toList(),
      };
}
...
String _resolveRoleLabel({
  ...
  final allRoles = teamMemberships.expand((m) => m.roles);
  if (allRoles.contains('team_admin')) return 'Team Admin';
  if (allRoles.contains('dispatcher')) return 'Dispatcher';
  if (allRoles.contains('responder')) return 'Responder';
